...
/*
void SwitchOn(void){
	HAL_GPIO_WritePin(GPIOE, GPIO_PIN_9, GPIO_PIN_SET); //dir ccw
	int threshold = 180;
	for(int i = 0; i < threshold; i++){
		HAL_GPIO_WritePin(GPIOF, GPIO_PIN_13, GPIO_PIN_SET); //step
		HAL_Delay(1);
		HAL_GPIO_WritePin(GPIOF, GPIO_PIN_13, GPIO_PIN_RESET); //step
	}
}

void SwitchOff(void){
	HAL_GPIO_WritePin(GPIOE, GPIO_PIN_9, GPIO_PIN_RESET); //dir cw
	int threshold = 180;
	for(int i = 0; i < threshold; i++){
		HAL_GPIO_WritePin(GPIOF, GPIO_PIN_13, GPIO_PIN_SET); //step
		HAL_Delay(1);
  ...
  */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  else if(state == 2 && TIM4->CNT > 50000){
		  next_state = 0;
